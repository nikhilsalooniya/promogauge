...
export const validatePhoneNumber = (phone: string, countryCode: string): { isValid: boolean; message?: string } => {
  ...
  if (cleaned.startsWith('+')) {
    if (!/^\+\d+$/.test(cleaned)) {
      return { isValid: false, message: 'Phone number should contain only digits after +' };
    }
    
    // Check minimum length (country code + at least 4 digits)
    if (cleaned.length < 6) {
      return { isValid: false, message: 'Phone number is too short' };
    }
  } else {
    // If no +, check it's only digits
    if (!/^\d+$/.test(cleaned)) {
      return { isValid: false, message: 'Phone number should contain only digits' };
    }
    
    // Check minimum length for local number
    if (cleaned.length < 4) {
      return { isValid: false, message: 'Phone number is too short' };
    }
  }

  // Check if it matches the expected country dial code
  const country = getCountryByCode(countryCode);
  if (country && cleaned.startsWith('+') && !cleaned.startsWith(country.dialCode.replace(/\s/g, ''))) {
    return { 
      isValid: false, 
      message: `Phone number should start with ${country.dialCode} for ${country.name}` 
    };
  }
...
export const formatPhoneWithCountryCode = (phone: string, countryCode: string): string => {
  ...
  // If already has +, return as is (user formatted it themselves)
  if (cleaned.startsWith('+')) {
    return cleaned;
  }
